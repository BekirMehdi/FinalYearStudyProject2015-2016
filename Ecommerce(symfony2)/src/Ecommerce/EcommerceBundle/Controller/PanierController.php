...
class PanierController extends Controller
{
    ...
    public function articlesAction()
    {
        ...
        return $this->render('EcommerceBundle:Default:panier/modulesUsed/articles.html.twig', array('articles' => $articles));
    }
    ...
    public function ajouterAction($id)
    {
     ...
     public function ajouterPremiumAction($idP)
    {
        $session = $this->getRequest()->getSession();
        ...
        if (array_key_exists($idP, $panier)) {
            if ($this->getRequest()->query->get('qte') != null) $panier[$idP] = $this->getRequest()->query->get('qte');
            $this->get('session')->getFlashBag()->add('success','Quantité modifié avec succès');
        } else {
            if ($this->getRequest()->query->get('qte') != null)
                $panier[$idP] = $this->getRequest()->query->get('qte');
            else
                $panier[$idP] = 1;
    ...
    public function panierAction()
    {
     ...
     public function panierHoverAction()
    {
        $session = $this->getRequest()->getSession();
        if (!$session->has('panier'))
            $articles = 0;
        ...
        return $this->render('EcommerceBundle:Default:panier/layout/panierHover.html.twig', array('articles' => $articles,'produits' => $produits,
                                                                                             'panier' => $session->get('panier')));
    }

      public function panierVerticalAction()
    {
        $session = $this->getRequest()->getSession();
        if (!$session->has('panier'))
            $articles = 0;
        ...
        return $this->render('EcommerceBundle:Default:panier/layout/panierVertical.html.twig', array('articles' => $articles,'produits' => $produits,
                                                                                             'panier' => $session->get('panier')));
    }
